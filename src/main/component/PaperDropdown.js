...
const PaperDropdown = forwardRef((props, ref) => {
    ...
    const setActive = useCallback((currentValue) => {
        ...
        if (multiSelect) {
            ...
            if (valueIndex === -1) {
                ...
                setValue([...value].filter((value) => value !== currentValue));
            }
    ...
    return (<Menu visible={visible} onDismiss={onDismiss} theme={theme} anchor={<TouchableRipple ref={ref} onPress={showDropDown} onLayout={onLayout} accessibilityLabel={accessibilityLabel}>
        <View pointerEvents={"none"}>
            <TextInput value={displayValue} mode={mode} label={label} placeholder={placeholder} placeholderTextColor={'gray'} pointerEvents={"none"} theme={theme} right={<TextInput.Icon icon={visible ? "menu-up" : "menu-down"} />} {...inputProps} />
        </View>
    </TouchableRipple>} style={{
        maxWidth: inputLayout?.width,
        width: inputLayout?.width,
        marginTop: inputLayout?.height - 0,
        ...dropDownStyle,
    }}>
